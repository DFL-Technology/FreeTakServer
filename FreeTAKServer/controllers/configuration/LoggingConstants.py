...
class LoggingConstants:
    def __init__(self):
        # main logging config
        self.PARENTPATH = MainConfig.LogFilePath

        self.LOGFORMAT = '%(levelname)s : %(asctime)s : %(filename)s:%(lineno)d : %(message)s'
        self.LOGNAME = 'FTS'

        self.ERRORLOG = str(PurePath(self.PARENTPATH, f"{self.LOGNAME}_error.log"))
        self.DEBUGLOG = str(PurePath(self.PARENTPATH, f"{self.LOGNAME}_debug.log"))
        self.INFOLOG = str(PurePath(self.PARENTPATH, f"{self.LOGNAME}_info.log"))
        self.HTTPLOG = str(PurePath(self.PARENTPATH, f"{self.LOGNAME}_http.log"))
        self.DELIMITER = ' ? '
